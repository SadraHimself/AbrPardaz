...
class LogService:
    ...
    async def log_timeweb_unpaid(self, hostname: str) -> None:
        await self._send(
            "server",
            f"⛔ <b>سرور تایم‌وب «پرداخت‌نشده» ماند</b>\n\n"
            f"سرور <code>{hostname}</code> ساخته شد ولی در وضعیت <b>no_paid</b> ماند؛ "
            "لغو شد و وجه کاربر کامل برگشت.\n\n"
            "⚠️ موجودیِ حساب مشکلی ندارد — یعنی حالتِ پرداختِ اکانتِ تایم‌وب روی "
            "«فاکتوری/دستی» است. تا این درست نشود، <b>هیچ سروری از ربات ساخته "
            "نمی‌شود</b>.\n"
            "راه‌حل: در پنل تایم‌وب پرداختِ خودکار از بالانس را روشن کن، یا از "
            "پشتیبانی بخواه اکانت را به «پرداخت از بالانس» تغییر دهد؛ و فاکتورهای "
            "Unpaid موجود را پرداخت کن.",
        )
