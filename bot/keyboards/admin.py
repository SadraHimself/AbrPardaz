...
def provider_types_kb() -> InlineKeyboardMarkup:
    ...
    builder.button(text="روت (RootVDS)", callback_data="admin:rootvds")
    builder.button(text="اسکیل‌وی (Scaleway)", callback_data="admin:scaleway")
...
def provider_detail_kb(provider_id: int, is_active: bool = True, strict_kyc: bool = False,
    ...
    # بازنگاشت مخصوص ویرچولایزور است (این کیبورد فقط برای همان استفاده می‌شود)
    builder.button(text="بازنگاشت سرورها", callback_data=f"admin:remap:{provider_id}")
