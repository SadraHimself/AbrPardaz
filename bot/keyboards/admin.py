...
def providers_list_kb(providers: list) -> InlineKeyboardMarkup:
    ...
    for p in providers:
        ...
        kyc = " 🔒" if p.strict_kyc else ""
        builder.button(text=f"{status} {p.name}{kyc}", callback_data=f"admin:prov:{p.id}")
...
def provider_detail_kb(provider_id: int, is_active: bool = True, strict_kyc: bool = False) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✏️ نام", callback_data=f"admin:prov_edit:{provider_id}:name")
    builder.button(text="✏️ URL پنل", callback_data=f"admin:prov_edit:{provider_id}:url")
    ...
    builder.button(text="👤 User ID", callback_data=f"admin:prov_edit:{provider_id}:uid")
...
def plans_categories_kb(categories: list[str]) -> InlineKeyboardMarkup:
    ...
    for cat in categories:
        builder.button(text=f"📁 {cat}", callback_data=f"admin:plans_cat:{cat}")
...
def plans_in_category_kb(plans: list, category: str) -> InlineKeyboardMarkup:
    ...
    for p in plans:
        status = "✅" if p.is_active else "❌"
        builder.button(
            text=f"{status} {p.display_name or p.name}",
...
def plan_detail_kb(plan_id: int, is_active: bool) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✏️ قیمت ساعتی", callback_data=f"admin:plan_edit:{plan_id}:price_hourly")
    builder.button(text="✏️ قیمت ماهانه", callback_data=f"admin:plan_edit:{plan_id}:price_monthly")
    builder.button(text="✏️ RAM", callback_data=f"admin:plan_edit:{plan_id}:ram")
    builder.button(text="✏️ CPU", callback_data=f"admin:plan_edit:{plan_id}:cpu")
    builder.button(text="✏️ Disk", callback_data=f"admin:plan_edit:{plan_id}:disk")
    builder.button(text="✏️ Bandwidth", callback_data=f"admin:plan_edit:{plan_id}:bandwidth")
    builder.button(text="✏️ موقعیت", callback_data=f"admin:plan_edit:{plan_id}:location")
    builder.button(text="✏️ دسته‌بندی", callback_data=f"admin:plan_edit:{plan_id}:category")
    builder.button(text="✏️ نام نمایشی", callback_data=f"admin:plan_edit:{plan_id}:display_name")
    builder.button(text="✏️ Plan ID ویرچو", callback_data=f"admin:plan_edit:{plan_id}:provider_plan_id")
    builder.button(text="📦 ریز-محصولات", callback_data=f"admin:subproducts:{plan_id}")
...
def discounts_list_kb(codes: list) -> InlineKeyboardMarkup:
    ...
    for c in codes:
        ...
        user_tag = "👤" if c.user_id else ""
        builder.button(
            text=f"{status}{user_tag} {c.code} — {c.discount_percent:.0f}%",
...
def discount_detail_kb(disc_id: int, is_active: bool) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✏️ درصد تخفیف", callback_data=f"admin:disc_edit:{disc_id}:percent")
    builder.button(text="✏️ تاریخ انقضا", callback_data=f"admin:disc_edit:{disc_id}:expires_at")
    builder.button(text="✏️ حداکثر استفاده", callback_data=f"admin:disc_edit:{disc_id}:max_uses")
    toggle_text = "🔴 غیرفعال" if is_active else "🟢 فعال"
...
def users_list_kb(users: list, page: int = 0, total: int = 0) -> InlineKeyboardMarkup:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="◀️ قبلی", callback_data=f"admin:users_page:{page-1}"))
    if (page + 1) * 20 < total:
        nav.append(InlineKeyboardButton(text="▶️ بعدی", callback_data=f"admin:users_page:{page+1}"))
    if nav:
        builder.row(*nav)
...
def settings_menu_kb() -> InlineKeyboardMarkup:
    ...
    builder.button(text="📋 شرایط و قوانین", callback_data="admin:setting:terms_text")
...
def channels_kb(channels: list[str]) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for ch in channels:
        builder.button(text=f"🗑 {ch}", callback_data=f"admin:ch_del:{ch.replace(':', '_')}")
...
def price_adj_categories_kb(categories: list[str]) -> InlineKeyboardMarkup:
    ...
    for cat in categories:
        builder.button(text=f"📁 {cat}", callback_data=f"admin:price_cat:{cat}")
...
def subproducts_kb(plan_id: int, sub_products: list) -> InlineKeyboardMarkup:
    ...
    for sp in sub_products:
        ...
        builder.button(
            text=f"{status} {sp.name} — {sp.price:,.0f}T",
...
def subprod_detail_kb(sp_id: int, plan_id: int, is_active: bool) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✏️ نام", callback_data=f"admin:subprod_edit:{sp_id}:name")
    builder.button(text="✏️ قیمت", callback_data=f"admin:subprod_edit:{sp_id}:price")
    builder.button(text="✏️ مقدار", callback_data=f"admin:subprod_edit:{sp_id}:value")
...
def providers_select_kb(providers: list) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for p in providers:
        builder.button(text=f"🖥 {p.name}", callback_data=f"admin:plan_prov:{p.id}")
