"""تنظیمات سراسری RootVDS — سود ساعتی/ماهانه، گروه مقصد، انتخاب اکانت (تک-اکانتی).

مدل (تصمیم پروژه 2026-07-26): RootVDS تک-اکانتی و کاملاً جدا از تایم‌وب است.
قیمت تعرفه‌ها ماهانه به «روبل» است؛ خرید ساعتی = ماهانه ÷ ۷۲۰ (فرمول چهارچوب) —
در provider محاسبه و در extra_data پلن ذخیره می‌شود (cost_hourly/cost_monthly).
فروش = خرید × (۱+سود٪) — ساعتی و ماهانه جدا. نرخ روبل→تومان: همان نرخ خودکارِ
Navasan (np_rub_to_irt_rate) که تایم‌وب استفاده می‌کند.

⚠️ درسِ تایم‌وب: auto-hide موجودی نداریم — خطاهای ظرفیتِ RootVDS صریح‌اند و
شکستِ ساخت فقط برگشتِ وجه می‌دهد؛ موجود/ناموجودی فقط دستی از پنل ادمین.
"""
...
_KEY_MH = "rootvds_margin_hourly"
_KEY_MM = "rootvds_margin_monthly"
_KEY_GROUP = "rootvds_group"
_DEFAULT_GROUP = "RootVDS"
...
async def apply_margins_to_catalog(session: AsyncSession) -> int:
    """قیمت فروش پلن‌های RootVDS = قیمت خرید × (۱+سود٪) — ساعتی و ماهانه + فعال‌سازی."""
...
async def get_account(session: AsyncSession) -> ProviderAccount | None:
    """تنها اکانت RootVDS (فعال یا نه) — برای پنل ادمین."""
...
async def pick_account(session: AsyncSession) -> ProviderAccount | None:
    """اکانت فعال RootVDS اگر زیر لیمیت VM دستی باشد (تک-اکانتی).
    API سقف نمی‌دهد → مصرف از DB خودمان شمرده می‌شود (الگوی جیکور/تایم‌وب)."""
