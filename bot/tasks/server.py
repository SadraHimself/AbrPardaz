...
@app.task(name="bot.tasks.server.sync_virtualizor_os", bind=True, max_retries=1)
def sync_virtualizor_os(self):
    """هر ۳۰ دقیقه: قالب‌های OS ویرچولایزور را با پنل تطبیق می‌دهد.

    قالب‌ها روی پنل حذف/اضافه/تغییرنام می‌شوند. اگر پلنی osid پین‌شده داشته باشد
    که دیگر وجود ندارد، ساخت سرور با آن پلن خطا می‌دهد — پس پین باطل پاک می‌شود
    (فلوی خرید آن‌وقت لیستِ زنده را نشان می‌دهد) و به ادمین گزارش می‌رود."""
    async def _do():
        ...
        async with AsyncSessionFactory() as session:
            ...
            try:
                for account in accounts:
                    ...
                    if not live:
                        continue          # لیست خالی مشکوک است؛ چیزی پاک نکن
                    ids = {str(o.get("id")) for o in live}
                    ...
                    for p in plans:
                        ...
                        pinned = extra.get("osid")
                        if pinned and str(pinned) not in ids:
                            extra.pop("osid", None)
                            p.extra_data = extra
                            stale.append((p.display_name or p.name, pinned))
                    if stale:
                        await session.commit()
                        _lines = "\n".join(f"• {n} (osid {o})" for n, o in stale[:15])
                        await log._send(
                            "server",
                            f"⚠️ <b>قالب OS نامعتبر</b>\n\n"
                            f"سرویس‌دهنده: {account.name}\n"
                            f"سیستم‌عاملِ پیش‌فرضِ این محصولات روی پنل دیگر وجود ندارد "
                            f"و پاک شد (خریدار از لیست زنده انتخاب می‌کند):\n{_lines}")
                await session.commit()
            finally:
