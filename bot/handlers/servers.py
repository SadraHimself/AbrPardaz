    ...
    async with AsyncSessionFactory() as session:
        try:
            ...
            try:
                ...
                # سشن را قبل از برگشت وجه پاک کن تا اگر ساختِ ناموفق تراکنش را
                # کثیف کرده باشد، credit/commit حتماً اجرا شود — برگشت وجه هرگز
                # نباید بی‌صدا شکست بخورد (کسرِ اولیه از قبل commit شده).
                try:
