...
# متن مشترک قوانین — هم در پرامپت /start هم در دکمه «قوانین» منو
_RULES_HEADER = '‏<tg-emoji emoji-id="5258328383183396223">📋</tg-emoji> <b>قوانین و مقررات استفاده از سرویس</b>\n\n'
_RULES_BODY = (
    "• هرگونه فعالیت غیرقانونی از جمله فیشینگ، اسپم، حملات سایبری و اسکن شبکه ممنوع است.\n"
    "• مسئولیت کامل محتوا و نحوه‌ی استفاده از سرور بر عهده‌ی کاربر است.\n"
    "• هزینه‌ی سرورهای ساعتی هر ساعت به‌صورت خودکار از کیف پول کسر می‌شود؛ "
    "در صورت اتمام موجودی، ۳ ساعت مهلت شارژ دارید و پس از آن سرویس حذف می‌شود.\n"
    "• سرورهای ماهانه در پایان دوره از کیف پول تمدید می‌شوند؛ عدم موجودی به معنای تعلیق سرویس است.\n"
    "• موجودی کیف پول قابل استرداد نیست.\n"
    "• تهیه‌ی نسخه‌ی پشتیبان از اطلاعات بر عهده‌ی کاربر است.\n"
    "• در صورت تخلف، سرویس بدون بازگشت وجه تعلیق یا حذف می‌شود.\n"
    "• پرداخت ریالی مستلزم احراز هویت و استفاده از کارت بانکی به نام خودتان است.\n"
)
...
async def _get_terms_text(session: AsyncSession) -> str:
    return await _get_setting(
        session, "terms_text",
        default=(
            _RULES_HEADER + _RULES_BODY
            + "• با انتخاب «قبول می‌کنم» تمامی موارد فوق را می‌پذیرید."
        ),
    )
