"""تنظیمات سراسری تایم‌وب — سود ماهانه، گروه مقصد، انتخاب اکانت (تک-اکانتی).

مدل: تایم‌وب تک-اکانتی است (مثل جیکور). قیمت تعرفه‌ها ماهانه به «روبل» است و در
extra_data پلن ذخیره می‌شود (cost_hourly/cost_monthly — ساعتی فقط داده‌ی هزینه
برای سرورهای قدیمی است). فروش = خرید × (۱+سود ماهانه٪).

⚠️ تایم‌وب **فقط ماهانه** فروخته می‌شود (تصمیم 2026-07-26: سقف روزانه‌ی IP برای
گردش فروش ساعتی مناسب نیست؛ فروش ساعتی → RootVDS). price_hourly پلن‌ها همیشه
None نگه داشته می‌شود (apply_margins + سینک). سرورهای ساعتیِ قدیمی از کپی قیمتِ
روی خودشان بیل می‌شوند (fallback در server_live_price).

نرخ روبل→تومان: خودکار از Navasan (np_rub_to_irt_rate) + قابل‌تنظیم دستی از پنل.
"""
